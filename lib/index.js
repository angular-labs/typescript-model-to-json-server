...
module.exports = {
  ...
  sendToJson: function(attrs, inserts, clean, modelPath, modelName) {
    let data = [];
    let value = null;
    ...
    if (clean) {
      fs.writeFile(modelPath, JSON.stringify(result), 'utf-8', function(err) {
        if (err) throw err;
      });
    } else {
      fs.appendFile(modelPath, JSON.stringify(result), 'utf-8', function (err) {
        if (err) throw err;
      });
    }
  }
